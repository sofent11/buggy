...
export function BugSection(props: {
  ...
  return (
    ...
    >
      ...
      <DataTable
        headers={['Bug', '来源', '负责人', '协作', '严重级别', '优先级', '状态', '更新时间', '操作']}
        emptyText="暂无 Bug"
        rows={rows.map((row) => [
          ...
          <Select value={row.status} onChange={(value) => props.mutate(() => api.updateBug(row.id, { status: value as never }), 'Bug 状态已更新')} values={bugStatuses} dictionaryType="bugStatus" />,
          shortDate(row.updatedAt),
          <div className="row-actions">
            {nextBugActions(row.status).map((action) => (
      ...
      {rows.length === 0 && (
        <EmptyState
          ...
          action={<button className="primary" type="button" onClick={() => setCreating(true)}><Plus size={16} /> 新建 Bug</button>}
        />
      )}
      <BugDrawer title="新建 Bug" open={creating} requirements={props.requirements} cases={props.cases} plans={props.plans} bugs={props.rows} users={props.users} onClose={() => setCreating(false)} onSubmit={async (form) => {
        await props.mutate(() => api.createBug(bugPayload(form, props.projectId)), 'Bug 已创建');
        setCreating(false);
      }} />
      <BugDrawer title="编辑 Bug" row={editingRow || undefined} open={Boolean(editing)} requirements={props.requirements} cases={props.cases} plans={props.plans} bugs={props.rows} users={props.users} onClose={() => setEditing(null)} onSubmit={async (form) => {
      ...
      <HookForm onSubmit={async (form) => props.onSubmit(form)}>
        {(register) => (
          <>
            ...
            {props.row && <BugCollaboration row={props.row} onComment={props.onComment} onAttachment={props.onAttachment} />}
...
function BugCollaboration(props: { row: Bug; onComment?: (body: string) => Promise<void>; onAttachment?: (attachment: { name: string; url: string }) => Promise<void> }) {
