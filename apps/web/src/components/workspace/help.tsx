...
const helpSections: HelpSection[] = [
  {
    ...
    title: '快速开始',
    icon: BookOpen,
    summary: '从创建项目到得到第一份质量报告的最短路径。',
    steps: [
      '在项目页新建项目，填写项目名称、代号和描述。',
      '打开项目行的成员入口，添加测试、开发、只读成员或设置负责人。',
      '创建迭代和需求，给需求设置负责人、优先级和状态。',
      '在用例库为需求补充测试用例和步骤。',
      '在执行页创建测试计划，选择本轮要执行的用例。',
      '记录执行结果，需要时从失败执行项直接创建 Bug。',
      '在报告页查看通过率、缺陷分布并导出数据。'
    ],
    tips: ['先选择当前项目再进入迭代、需求、用例、执行、Bug 和报告模块。', '顶部全局搜索会过滤当前工作区内的需求、用例、计划和 Bug。'],
  ...
  },
  {
    ...
    tips: ['迭代目标建议写成本轮验证范围或交付目标。', '归档迭代适合保留历史数据，不影响报告统计的项目范围。'],
    keywords: ['迭代', '周期', '规划', '归档', '目标']
  },
  {
    id: 'requirements',
    title: '需求管理',
    ...
    keywords: ['需求', '负责人', '优先级', '状态', 'Lark', '日报']
  },
  {
    id: 'cases',
    title: '用例库',
  ...
  },
  {
    ...
    title: '统计报告',
    icon: BarChart3,
    summary: '查看需求完成率、用例准备率、执行通过率、缺陷分布并导出数据。',
    steps: [
      '报告页展示当前项目的核心质量指标和图表。',
      '可以打开 HTML 报告用于浏览或分享。',
      '支持导出需求、用例和 Bug 数据。',
      '总览页的风险信号来自执行失败/阻塞、活跃 Bug 和阻塞需求。'
    ],
    tips: ['报告范围始终跟随当前项目。', '如果报告为空，先检查当前项目是否已有需求、用例、执行计划或 Bug。'],
    keywords: ['报告', '导出', '图表', '通过率', '质量统计']
  },
  {
    id: 'settings',
    title: '系统配置与导入',
    ...
    <Drawer title="帮助中心" subtitle="按当前功能整理的操作指南、权限说明和排查入口" open={props.open} onClose={props.onClose} size="full">
      <div className="help-center">
        ...
        <section className="help-shortcuts" aria-label="常用排查">
          ...
          <article>
            <Activity size={17} />
            <strong>报告没有数据？</strong>
            <span>确认当前项目下已有需求、用例、执行计划或 Bug。</span>
          </article>
        </section>
      </div>
    </Drawer>
