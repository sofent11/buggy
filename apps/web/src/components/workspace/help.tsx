...
const helpSections: HelpSection[] = [
  {
    ...
    steps: [
      ...
      '记录执行结果，需要时从失败执行项直接创建缺陷。',
      '在迭代或需求列表打开报告，查看阶段质量或验收证据。'
    ],
    tips: ['先选择当前项目再进入迭代、需求、用例、执行和缺陷模块。', '顶部全局搜索会过滤当前工作区内的需求、用例、计划和缺陷。'],
  ...
  },
  {
    ...
    steps: [
      ...
      '删除项目在更多操作中确认执行，会删除项目下迭代、需求、用例、执行计划、缺陷和报告数据。'
    ],
  ...
  },
  {
    ...
    steps: [
      ...
      '需求可以绑定到迭代，迭代页会汇总关联需求、用例、执行和活跃缺陷。',
  ...
  },
  {
    ...
    title: '测试执行',
    icon: Activity,
    summary: '组织测试轮次、选择执行用例、记录执行结果并创建缺陷。',
    steps: [
      '创建测试计划时选择计划名称、轮次、迭代、需求范围和本轮用例。',
      '执行项可以快速标记通过、失败或阻塞。',
      '使用“记录”补充实际结果。',
      '失败或阻塞时可从执行项直接创建缺陷，系统会保留执行来源。'
    ],
    tips: ['每一轮回归建议使用独立测试计划，便于比较通过率。', '实际结果写清环境、数据和观察现象，会让缺陷定位更快。'],
    keywords: ['执行', '测试计划', '轮次', '结果', '通过率', '建缺陷']
  },
  {
    id: 'bugs',
    title: '缺陷管理',
    ...
    steps: [
      '缺陷可以绑定需求、用例和测试计划，也可以独立创建。',
      '列表支持按状态、严重级别、负责人和关键字筛选。',
      '编辑缺陷时记录复现步骤、实际结果、期望结果和责任人。',
      '已验证和已关闭的缺陷不再计入活跃缺陷。'
    ],
    tips: ['S0/S1 严重缺陷会在缺陷页指标中重点提示。', '重新打开用于标记修复后复现或验证失败的缺陷。'],
  ...
  },
  {
    ...
    steps: [
      '下载需求、用例或缺陷模板后，可按模板表头批量导入。',
      'Excel 导入支持需求、用例、缺陷和执行结果。',
      '管理员可以维护用户系统角色和账号状态。',
      '字典配置可维护需求状态、执行状态、缺陷状态、优先级和严重级别。'
    ],
