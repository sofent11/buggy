...
import type { Bug, Iteration, Requirement, TestCase, TestPlan, TestRunItem } from '@buggy/shared-types';
  ...
  return (
    ...
    >
      <Toolbar>
        ...
        <Select value={status} onChange={setStatus} values={planStatuses} emptyLabel="全部状态" />
      ...
      {rows.length === 0 && <EmptyState text="暂无测试计划" />}
      <PlanCreateDrawer
        open={creating}
  ...
  return (
    <article className="plan-card">
      <header>
        ...
        <StatusBadge value={props.plan.status} />
        ...
        <HookForm defaultValues={{ name: props.plan.name, round: props.plan.round, status: props.plan.status }} onSubmit={async (form) => {
          ...
          {(register) => (
            <>
              ...
              <Field><FieldLabel>状态</FieldLabel><Select name="status" register={register} values={planStatuses} defaultValue={props.plan.status} /></Field>
    ...
    <div className="row-actions">
      ...
      <Drawer title="记录执行结果" subtitle={props.item.caseTitle} open={editing} onClose={() => setEditing(false)}>
        <HookForm defaultValues={{ status: props.item.status, actualResult: props.item.actualResult || '' }} onSubmit={async (form) => {
          await props.mutate(() => api.updateRunItem(props.planId, props.item.id, { status: text(form, 'status'), actualResult: text(form, 'actualResult') }), '执行结果已更新');
          setEditing(false);
        }}>
          {(register) => (
            <>
              <Field><FieldLabel>执行状态</FieldLabel><Select name="status" register={register} values={runStatuses} defaultValue={props.item.status} /></Field>
