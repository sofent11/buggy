...
export class TestPlanService {
  ...
  async create(dto: CreateTestPlanDto, user?: SessionUser): Promise<TestPlan> {
    ...
    const runItems = cases.map((testCase) => ({
      ...
      caseVersion: testCase.version || 'v1',
      caseUpdatedAt: (testCase as TestCaseEntity & { updatedAt?: Date }).updatedAt,
      status: 'untested',
      actualResult: '',
      bugIds: [],
      stepResults: []
    ...
    return {
      ...
      runItems: row.runItems.map(
        (item): TestRunItem => ({
          ...
          caseVersion: item.caseVersion || 'v1',
          caseUpdatedAt: item.caseUpdatedAt?.toISOString(),
          status: item.status,
